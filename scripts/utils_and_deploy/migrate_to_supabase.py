...
if ENV_PATH.exists():
    ...
    print(
        f"[AVISO] Arquivo .env não encontrado em {ENV_PATH}. Usando variáveis de ambiente globais."
    )
...
def migrate():
    if not SUPABASE_DB_URL or "SEU_PROJETO" in SUPABASE_DB_URL:
        ...
        print(
            "Edite o arquivo backend/.env e informe a Connection URI do seu banco Supabase."
        )
        sys.exit(1)
    ...
    print(
        f"Destino (Supabase): {SUPABASE_DB_URL.split('@')[-1]}"
    )  # Printa apenas host e porta sem credenciais
    ...
    try:
        ...
        print(
            f"Encontrados {len(materials)} materiais no SQLite. Iniciando inserção..."
        )
        ...
        print(
            f"Encontradas {len(estruturas)} estruturas no SQLite. Iniciando inserção..."
        )
        ...
        lite_cur.execute(
            "SELECT id, estrutura_id, material_codigo, material_descricao, quantidade FROM estrutura_materiais"
        )
        em_rows = lite_cur.fetchall()
        print(
            f"Encontradas {len(em_rows)} relações estrutura-materiais no SQLite. Iniciando inserção..."
        )
        ...
        pg_cur.execute(
            "SELECT setval('estruturas_id_seq', COALESCE((SELECT MAX(id)+1 FROM estruturas), 1), false)"
        )
        pg_cur.execute(
            "SELECT setval('estrutura_materiais_id_seq', COALESCE((SELECT MAX(id)+1 FROM estrutura_materiais), 1), false)"
        )
